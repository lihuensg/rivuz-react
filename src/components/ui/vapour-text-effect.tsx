...
const SeoElement = memo(({ tag = Tag.P, texts }: { tag: TagType, texts: string[] }) => {
  const style = useMemo(() => ({
  ...
  const safeTag = (Object.values(Tag) as string[]).includes(tag as string) ? tag : "p";
...
const renderCanvas = ({
  ...
  const { particles, textBoundaries } = createParticles(ctx, canvas, currentText, textX, textY, font, color, framerProps.alignment || "left", framerProps.font?.letterSpacing);
  particlesRef.current = particles;
  canvas.textBoundaries = textBoundaries;
};

const createParticles = (
  ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, text: string, textX: number, textY: number, font: string, color: string, alignment: "left" | "center" | "right", letterSpacing: string = "normal"
) => {
  const particles = [];
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ctx.fillStyle = color;
  ...
  for (let y = 0; y < canvas.height; y += sampleRate) {
    for (let x = 0; x < canvas.width; x += sampleRate) {
      ...
      if (alpha > 0) {
        ...
        // LOGO HACK: Colorize the dot naturally!
        if (text === "RIVUZ.") {
           const dotWidth = ctx.measureText(".").width;
           if (x > textBoundaries.right - dotWidth * 1.8) {
               r = 0;
               g = 179;
               b = 255;
           }
        ...
        const particle = {
          ...
          opacity: originalAlpha, originalAlpha, velocityX: 0, velocityY: 0, angle: 0, speed: 0,
        };
